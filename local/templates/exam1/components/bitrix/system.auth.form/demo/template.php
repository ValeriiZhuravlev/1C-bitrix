	...
	<ul>
		<li class="att popup-wrap"><a id="hd_singin_but_open" href="" class="btn-toggle"><?=GetMessage("AUTH_LOGIN_LINK_TEXT")?></a>
			<form class="frm-login popup-block" name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
				...
				<div class="frm-row">
					<a href="/s2<?=$arParams['FORGOT_PASSWORD_URL']?>" class="btn-forgot"><?=GetMessage("AUTH_FORGOT_PASSWORD_2")?></a>
				</div>
		...
		<li><a href="/s2<?=$arParams['REGISTER_URL']?>"><?=GetMessage("AUTH_REGISTER")?></a></li>
	</ul>
</nav>
<?
} else {
?>
                    <nav class="menu-block">
                        <ul>
                            <li>
                                <a href="/s2<?=$arParams['PROFILE_URL']?>" ><?=$USER->GetFullName()?> [<?=$USER->GetLogin()?>]</a>
                            </li>
